...
- **`helm upgrade` Bitrix: `webhook.cert-manager.io` timeout / Internal error** — webhook не готов. Плейбук опрашивает **`readyReplicas`**; около попытки **18** — **`rollout restart`**. Если в логах вечно **`readyReplicas=0`**: раньше **cert-manager** мог быть привязан к **master** вместе с **ingress** — задайте **`cert_manager_on_control_plane: false`** (см. `all.yml.example`) и снова **`--tags bitrix`** (плейбук сам делает **`helm upgrade` cert-manager** и сбрасывает `nodeSelector` у webhook). Отключить ожидание: **`bitrix_wait_cert_manager_webhook: false`**. Параметры: **`cert_manager_webhook_poll_*`**, **`bitrix_helm_retries`**. Вручную: `kubectl describe pod -n cert-manager -l app.kubernetes.io/name=webhook`.
