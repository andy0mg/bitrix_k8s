...
- **Все поды Bitrix в `Pending`, PVC postgres/sphinx `Pending`** — нет **StorageClass** по умолчанию: в Ansible-шаблоне **`global.storageClass.rwo`** берётся как **`bitrix_storage_rwx`**, если **`bitrix_storage_rwo`** пусто. Плюс на одной control-plane ноде с **taint** чарт получает **tolerations**, если в `group_vars` **`ingress_nginx_on_control_plane: true`**. После `git pull` выполните **`helm upgrade`** (плей `--tags bitrix`) или снимите taint / подключите workers.  
