...
- **Helm: `Chart.yaml file is missing`, хотя файл на диске есть** — часто **Helm из Snap** + репозиторий в **`/root/`**. Поставьте Helm через `get-helm-3` или перенесите репо в `/opt/...`.  
