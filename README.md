...
- **`helm upgrade` Bitrix: `webhook.cert-manager.io` timeout / Internal error** — webhook ещё не принимает запросы. Плейбук **`--tags bitrix`** ждёт **`cert-manager-webhook`**, при сбое делает **`rollout restart`** и повторное ожидание, затем **до 5 повторов** самого Helm (см. `cert_manager_webhook_*`, **`bitrix_helm_retries`** в `all.yml.example`). Вручную: `kubectl get pods -n cert-manager`, `kubectl rollout restart deployment/cert-manager-webhook -n cert-manager`.
