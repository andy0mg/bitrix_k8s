...
- **Helm: `Chart.yaml file is missing`, хотя файл на диске есть** — часто **Helm из Snap** + репозиторий в **`/root/`**. Поставьте Helm через `get-helm-3` или перенесите репо в `/opt/...`. Плейбук Bitrix ставит чарт через **`helm package` → `.tgz`**; если падает и это — `helm show chart /путь/helm/bitrix` и `type -a helm` для диагностики.  
