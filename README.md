...
- **`helm upgrade` Bitrix: `webhook.cert-manager.io` timeout / Internal error** — webhook не готов. Плейбук опрашивает **`readyReplicas`**; около попытки **18** — **`rollout restart`**. Если в логах вечно **`readyReplicas=0`** у **webhook**: сначала должны стать Ready **controller** и **cainjector** того же Helm-релиза; плейбук ждёт все три (`$REL`, `$REL-cainjector`, `$REL-webhook`). При **`k8s_join_workers: true`** **cert-manager** по умолчанию без привязки к master (см. шаблон values). В конце при ошибке выводится **`describe`** webhook-pod и **events**. Отключить ожидание: **`bitrix_wait_cert_manager_webhook: false`**. Параметры: **`cert_manager_webhook_poll_*`**, **`bitrix_helm_retries`**. Вручную: `kubectl describe pod -n cert-manager -l app.kubernetes.io/name=webhook`.
