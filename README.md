...
- **`helm upgrade` Bitrix: `webhook.cert-manager.io` timeout / Internal error** — webhook не готов. Плейбук опрашивает **`readyReplicas`** (не длинный `kubectl wait`). Около попытки **18** — **`rollout restart`** webhook. Отключить ожидание: **`bitrix_wait_cert_manager_webhook: false`**. Параметры: **`cert_manager_webhook_poll_*`** в `all.yml.example`; затем **повторы Helm** (**`bitrix_helm_retries`**). Вручную: `kubectl get deploy,pods -n cert-manager`, `kubectl rollout restart deployment/cert-manager-webhook -n cert-manager`.
